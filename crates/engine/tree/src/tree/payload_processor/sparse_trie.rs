...
pub(super) struct SparseTrieCacheTask<A = SerialSparseTrie, S = SerialSparseTrie> {
    ...
    /// Cache of account proof targets that have already been dispatched.
    /// Maps target key to the minimum `min_len` value that has been requested.
    /// A lower `min_len` means more of the trie path is fetched, so we skip
    /// requests with `min_len` >= the cached value.
    fetched_account_targets: B256Map<u8>,
    /// Cache of storage proof targets that have already been dispatched per account.
    /// Maps (account, slot) to the minimum `min_len` value that has been requested.
