...
mod tests {
    ...
    async fn shared_socket_binds_both_families_on_one_port() {
        ...
        //
        // Probe IPv6 separately so a host that genuinely lacks it skips, while a same-port bind
        // failure (e.g. a regressed V6ONLY, which is what this guards) still fails loudly.
        let Ok(probe) = bind_socket("[::]:0".parse().unwrap()).await else { return };
        drop(probe);
    ...
    fn discv5_config_advertises_dual_stack_nat_endpoints() {
        ...
        assert_eq!(enr.udp4(), Some(45678));
        assert_eq!(enr.udp6(), Some(45678));
        // A discovery-only bootnode serves no RLPx, so TCP is advertised as 0 on every family.
        assert_eq!(enr.tcp4(), Some(0));
        assert_eq!(enr.tcp6(), Some(0));

        // The derived enode therefore renders as `…@<ip>:0?discport=<udp>`.
